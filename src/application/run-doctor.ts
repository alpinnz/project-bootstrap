...
async function checkFoundation(fs: FileSystem, root: string): Promise<ValidationCategory> {
  ...
  checks.push(
    await fileCheck(fs, root, '.project-bootstrap/constitution.md', 'Constitution'),
  );
  checks.push(
    await fileCheck(fs, root, '.project-bootstrap/project.yml', 'Project context'),
  );
...
async function checkDevelopment(
  ...
  checks.push({
    ...
    message: commands.includes('test')
      ? 'npm test available'
  ...
  checks.push({
    ...
    message: commands.includes('build')
      ? 'npm build available'
...
async function checkSecurity(fs: FileSystem, root: string): Promise<ValidationCategory> {
  ...
  checks.push({
    ...
    message: envPlayground && !envExample
